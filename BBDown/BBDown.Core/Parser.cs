...
public static partial class Parser
{
    ...
    private static async Task<string> GetPlayJsonAsync(string encoding, string aidOri, string aid, string cid, string epId, bool tvApi, bool intl, bool appApi, string qn = "0")
    {
        ...
        //课程接口: cheese 用 /pugv/player/web/playurl (无 /v2/), v2 端点对 cheese 返回 404
        if (cheese)
        {
            api = api.Replace("/pgc/player/web/v2/playurl", "/pugv/player/web/playurl")
                     .Replace("/pgc/", "/pugv/");
        }
        ...
        if (parsedResult.WebJsonString.Contains("\"dash\":{")) //dash
        {
            ...
            if (video != null)
            {
                foreach (var node in video)
                {
                    ...
                    ExtractDrmInfo(node, out v.bilidrmUri, out v.kid);
                    if (!parsedResult.VideoTracks.Contains(v)) parsedResult.VideoTracks.Add(v);
                }
            }
            ...
            if (audio != null)
            {
                foreach (var node in audio)
                {
                    ...
                    ExtractDrmInfo(node, out audioTrack.bilidrmUri, out audioTrack.kid);
                    parsedResult.AudioTracks.Add(audioTrack);
                }
            }

            // 标记 DRM 信息（cheese ClearKey）
            if (root.TryGetProperty("dash", out var dashNode)
                && dashNode.TryGetProperty("drm_type", out var drmTypeNode)
                && drmTypeNode.ValueKind == JsonValueKind.String)
            {
                var t = drmTypeNode.GetString();
                if (!string.IsNullOrEmpty(t)) parsedResult.DrmType = t;
            }
            parsedResult.IsDrm = parsedResult.VideoTracks.Any(t => t.kid != null)
                              || parsedResult.AudioTracks.Any(t => t.kid != null);
    ...
    /// <summary>
    /// 从 dash 流节点中提取 DRM 信息: bilidrm_uri 与 KID(取末段 32 位 hex)
    /// </summary>
    private static void ExtractDrmInfo(JsonElement node, out string? bilidrmUri, out string? kid)
    {
        bilidrmUri = null;
        kid = null;
        if (!node.TryGetProperty("bilidrm_uri", out var u) || u.ValueKind != JsonValueKind.String) return;
        var uri = u.GetString();
        if (string.IsNullOrEmpty(uri)) return;
        bilidrmUri = uri;
        var parts = uri.Split("//", StringSplitOptions.None);
        if (parts.Length < 2) return;
        var tail = parts[^1];
        if (tail.Length >= 32) kid = tail[..32].ToLowerInvariant();
    }
