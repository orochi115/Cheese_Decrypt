        ...
        string? firstEncoding, bool downloadDanmaku, BBDownDanmakuFormat[] downloadDanmakuFormats, string input, string savePathFormat, string lang, string aidOri, string apiType, DownloadTask? relatedTask = null)
    {
        ...
        try
        {
            ...
            if ((parsedResult.VideoTracks.Any() || parsedResult.AudioTracks.Any()) && !parsedResult.Clips.Any())   //dash
            {
                ...
                // DRM (cheese ClearKey) 解密: 在下载与混流之间替换原文件
                if (parsedResult.IsDrm)
                {
                    var drmKeys = BBDownDrm.ParseKeyArgs(myOption.Keys);
                    if (myOption.DrmAuto)
                    {
                        var node = string.IsNullOrEmpty(myOption.NodePath) ? "node" : myOption.NodePath;
                        if (!BBDownDrm.IsNodeAvailable(node))
                        {
                            LogError($"--drm-auto 需要 Node.js >=18, 但 '{node}' 不可执行 (用 --node-path 指定)。");
                            return;
                        }
                        var helper = !string.IsNullOrEmpty(myOption.DrmHelperPath) ? myOption.DrmHelperPath : BBDownDrm.ResolveHelperScriptPath();
                        if (helper == null || !File.Exists(helper))
                        {
                            LogError("找不到 bili-drm-helper.js, 请用 --drm-helper-path 指定。");
                            return;
                        }
                        var cookie = !string.IsNullOrEmpty(myOption.Cookie) ? myOption.Cookie : BBDown.Core.Config.COOKIE;
                        if (string.IsNullOrEmpty(cookie))
                        {
                            LogError("--drm-auto 需要登录态: 先 `BBDown login` 扫码登录, 或用 --cookie 提供 (至少含 SESSDATA)。");
                            return;
                        }
                        var kidsToFetch = new List<string>();
                        if (!string.IsNullOrEmpty(selectedVideo?.kid)) kidsToFetch.Add(selectedVideo!.kid!);
                        if (!string.IsNullOrEmpty(selectedAudio?.kid)) kidsToFetch.Add(selectedAudio!.kid!);
                        try
                        {
                            var auto = await BBDownDrm.AcquireKeysAutoAsync(kidsToFetch, cookie, node, helper);
                            // 手动 --key 优先, 自动取的只补全缺失项
                            foreach (var kv in auto) if (!drmKeys.ContainsKey(kv.Key)) drmKeys[kv.Key] = kv.Value;
                        }
                        catch (Exception ex)
                        {
                            LogError($"--drm-auto 失败: {ex.Message}");
                            return;
                        }
                    }
                    if (drmKeys.Count == 0)
                    {
                        LogError($"此视频为DRM加密(drm_type={parsedResult.DrmType ?? "?"}), 必须使用 --key kid:key 提供解密密钥, 或加 --drm-auto 由内置 helper 自动获取。" +
                                 $"视频KID={selectedVideo?.kid ?? "-"}, 音频KID={selectedAudio?.kid ?? "-"}");
                        return;
                    }
                    var mp4d = string.IsNullOrEmpty(myOption.Mp4decryptPath) ? "mp4decrypt" : myOption.Mp4decryptPath;
                    if (!BBDownDrm.IsAvailable(mp4d))
                    {
                        LogError("mp4decrypt 不可用, 请确认其在 PATH 或使用 --mp4decrypt-path 指定。");
                        return;
                    }
                    if (selectedVideo?.kid != null && !string.IsNullOrEmpty(videoPath) && File.Exists(videoPath))
                    {
                        var key = BBDownDrm.ResolveKeyForKid(selectedVideo.kid, drmKeys);
                        if (key == null) { LogError($"未找到视频KID {selectedVideo.kid} 对应的key, 请用 --key {selectedVideo.kid}:<keyhex> 提供"); return; }
                        var enc = videoPath + ".enc";
                        File.Move(videoPath, enc, true);
                        Log($"开始解密P{p.index}视频流 (kid={selectedVideo.kid})...");
                        var rc = BBDownDrm.Decrypt(mp4d, selectedVideo.kid, key, enc, videoPath);
                        if (rc != 0 || !File.Exists(videoPath) || new FileInfo(videoPath).Length == 0)
                        {
                            LogError($"视频流 mp4decrypt 解密失败, exit={rc}");
                            return;
                        }
                        File.Delete(enc);
                    }
                    if (selectedAudio?.kid != null && !string.IsNullOrEmpty(audioPath) && File.Exists(audioPath))
                    {
                        var key = BBDownDrm.ResolveKeyForKid(selectedAudio.kid, drmKeys);
                        if (key == null) { LogError($"未找到音频KID {selectedAudio.kid} 对应的key, 请用 --key {selectedAudio.kid}:<keyhex> 提供"); return; }
                        var enc = audioPath + ".enc";
                        File.Move(audioPath, enc, true);
                        Log($"开始解密P{p.index}音频流 (kid={selectedAudio.kid})...");
                        var rc = BBDownDrm.Decrypt(mp4d, selectedAudio.kid, key, enc, audioPath);
                        if (rc != 0 || !File.Exists(audioPath) || new FileInfo(audioPath).Length == 0)
                        {
                            LogError($"音频流 mp4decrypt 解密失败, exit={rc}");
                            return;
                        }
                        File.Delete(enc);
                    }
